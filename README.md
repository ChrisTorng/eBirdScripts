...
5. 日期、地點與時間沒有紅字後，按「開始填寫紀錄」；單一鳥種無法辨識時，其他已知鳥種仍會繼續填入。
6. 到鳥種頁後，助手會隱藏未觀察鳥種，並顯示包含日期、地點、時間及全部鳥種結果的核對表。鳥種依 eBird 頁面順序排列，繁殖代碼使用頁面實際選取的文字；未辨識或未成功填入的項目以紅字列出。
7. 可按「顯示未觀察到的鳥種項目」恢復完整清單，再人工檢查所有資料。助手不會自動送出；人工送出後仍會在該筆鳥單頁保留核對資訊。
