...
- 可直接篩選及選取 eBird 地點，並在 Tampermonkey 本機設定「地名簡稱 → eBird 地點、預設距離與人數」。
- 支援多種日期簡寫、常用鳥種簡稱、鳥名與數字相連的寫法，以及「唱歌」、「聽到」與「一對」等細節。
- 行動裝置預設收合，並可由右上角按鈕展開或收合。
- 盡量填完所有鳥種後，依 eBird 頁面順序列出完整核對表，並預設隱藏未觀察到的鳥種。
...
3. 已設定的地點簡稱會自動選到對應地點。新簡稱會用來篩選 eBird 地點，並自動展開「新增／管理地點」；確認地點後只需補上預設距離與人數。
4. 日期、地點與時間沒有紅字後，按「開始填寫紀錄」；單一鳥種無法辨識時，其他已知鳥種仍會繼續填入。
5. 到鳥種頁後，助手會隱藏未觀察鳥種，並顯示包含日期、地點、時間及全部鳥種結果的核對表。鳥種依 eBird 頁面順序排列，繁殖代碼使用頁面實際選取的文字；未辨識或未成功填入的項目以紅字列出。
6. 可按「顯示未觀察到的鳥種項目」恢復完整清單，再人工檢查所有資料；助手不會送出紀錄。
...
- 每個地名簡稱可設定 eBird 地點 ID、完整名稱、預設距離與人數。
